...
select StockQuantity, IIF(StockQuantity>100, 'Yes','No') as StQuan from Products_Discounted
...
select Price, iif(Price>1000, 'Expensive','Affordable') as PriceDescr from Products
...
CREATE TABLE Sales (SaleID INT PRIMARY KEY,ProductID INT,CustomerID INT,
...
Select SaleAmount, CASE 
when SaleAmount > 500 then 'Top Tier'
when SaleAmount between 200 and 500 then 'Mid Tier'
else 'Low Tier' 
end as Result From Sales

CREATE TABLE Orders (OrderID INT PRIMARY KEY,CustomerID INT,ProductID INT,
OrderDate DATE,Quantity INT,TotalAmount DECIMAL(10, 2),
