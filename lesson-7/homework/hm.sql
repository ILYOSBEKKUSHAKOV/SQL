...
select count(ProductName), Category from Products
Group by Category

select ProductID, sum(SaleId * SaleAmount) as TotalSalesAmount  from Sales
where ProductID = 7
Group by ProductID
...
select Avg(SaleAmount) as TotalSalePerCustomer from Sales

select DepartmentName, Count(EmployeeId) as NumberOfEmployees from Employees
Group by DepartmentName
Having Count(EmployeeID) > 5

select Category, sum(Price*StockQuantity) as TotalPriceForCategiry, 
Avg(Price) as ForEachCategory from Products
Group by Category

select ProductID, sum(SaleAmount) as TotalPriceForSaleID, 
Avg(SaleAmount) as ForEachSaleID from Sales
Group by ProductID

select count(EmployeeID) as HREmployees from Employees
where DepartmentName = 'HR'

select DepartmentName, Min(Salary) as LowestSalary, Max(Salary) as HighestSalary from Employees
Group by DepartmentName

select DepartmentName, AVG(Salary) as AverageSalaryPerDepartment from Employees
Group by DepartmentName

select DepartmentName, AVG(Salary) as AverageSalaryPerDepartment,
count(EmployeeID) as NumberOfEmployees from Employees
Group by DepartmentName
...
In the question 17 there is no necessary data for soving the problem

select SaleDate, Sum(SaleAmount) from Sales
Group by SaleDate

Select CustomerId, Count(OrderID) AS AmountofPlacedOrders from Orders
group by CustomerID
...
select CustomerID, Sum(SaleAmount) as TotalSaleAmount from Sales
Group by CustomerID
...
select DepartmentName, Sum(Salary) as TotalSalary, AVG(Salary) as AverageSalaryPerDepartment from Employees
Group by DepartmentName
...
select custid, sum(freight) as SumFreight from [TSQL2012].[Sales].[Orders]
Group by custid
having sum(freight) > 50

In the question 24 there is no necessary data for soving the problem

In the question 25 there is no necessary data for soving the problem
