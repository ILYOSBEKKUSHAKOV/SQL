...
select p.ProductName, o.OrderID, o.Quantity from Products p
join Orders o on o.ProductID = p.ProductID

select e.Name, d.DepartmentName from Departments d
join Employees e on e.DepartmentID = d.DepartmentID

select st.Name, en.CourseID from Students st
join Enrollments en on en.StudentID = st.StudentID

select p.PaymentID, o.ProductID from Payments p
join Orders o on o.OrderID = p.OrderID

select p.Productname, o.OrderID, p.Price from Products p
join Orders o on o.ProductID = p.ProductID
where p.Price > 100
...
select st.Name, c.CourseName, e.EnrollmentID from Students st
...
select o.ProductID, o.TotalAmount, p.PaymentID, p.Amount from Orders o
...
join Categories c on c.CategoryName = p.Category
where c.CategoryName in ('Electronics','Furniture')

select c.FirstName, c.LastName, s.SaleID, c.Country from Customers c
join Sales s on s.CustomerID = c.CustomerID
where c.Country = 'USA'

select c.FirstName, c.LastName, o.TotalAmount, c.Country from Customers c
join Orders o on o.CustomerID = c.CustomerID
where c.Country = 'Germany' and o.TotalAmount > 100

select e.Name, s.Name from Employees e
join Employees s on s.EmployeeID = e.EmployeeID
and s.DepartmentID <> e.DepartmentID

select p.ProductName, p.Price, pay.PaymentID, pay.Amount, o.Quantity from Products p
join Orders o on o.ProductID = p.ProductID
...
select e.Name as ManagerName, c.Name as UnderManager, e.Salary, c.Salary from Employees e
join Employees c on c.ManagerID = e.EmployeeID
where e.Salary <= c.Salary

select c.FirstName, c.LastName, o.OrderID, p.PaymentID from Customers c
join Orders o on o.CustomerID = c.CustomerID
join Payments p on p.OrderID = o.OrderID
